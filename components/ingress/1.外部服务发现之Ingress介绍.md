...
&#8195;&#8195;注意这里为何是23456而不是23457，因为这里是通过ingress设置的域名来访问的，宿主机的23456端口宿主机上traefik-ingress-controller-nginx-pod容器的80端口，然后再经过ingress代理到service对应的pod节点上，如果traefik-ingress-controller-nginx-pod设置了宿主机端口映射，那么可以省略23456端口，下面会讲到hostPort: 80参数的使用）
